    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                switch (operation) {
                    case 'toggle':
                        const moduleCategory = this.getNodeParameter('moduleCategory', i);
                        const moduleName = this.getNodeParameter('moduleName', i);
                        // Use Minescript to send Meteor command
                        command = `\\eval "minescript.execute('.toggle ${moduleName}')"`;
                        break;
                    case 'configure':
                        const configModuleCategory = this.getNodeParameter('moduleCategory', i);
                        const configModuleName = this.getNodeParameter('moduleName', i);
                        const settingName = this.getNodeParameter('settingName', i);
                        ...
                        break;
                    case 'status':
                        const statusModuleCategory = this.getNodeParameter('moduleCategory', i);
                        const statusModuleName = this.getNodeParameter('moduleName', i);
                    ...
                    case 'list':
                        ...
                        break;
                    case 'saveProfile':
                        const saveProfileName = this.getNodeParameter('profileName', i);
                        command = `\\eval "minescript.execute('.profiles save ${saveProfileName}')"`;
                        break;
                    case 'loadProfile':
                        const loadProfileName = this.getNodeParameter('profileName', i);
                ...
                if (advancedOptions.sendHudUpdate && (operation === 'toggle' || operation === 'configure')) {
                    await this.makeMinescriptRequest(baseUrl, credentials, '\\eval "minescript.execute(\'.hud reload\')"');
                }
                returnData.push({
                    json: {
                        operation,
