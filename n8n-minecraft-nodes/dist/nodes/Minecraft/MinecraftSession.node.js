...
exports.MinecraftSession = void 0;
...
class MinecraftSession {
            ...
            try {
                ...
                switch (operation) {
                    ...
                    case 'getStatus':
                        ...
                        break;
                    case 'listenEvents':
                        const eventTypes = this.getNodeParameter('eventTypes', i);
                        const eventTimeout = this.getNodeParameter('eventTimeout', i);
                        result = await this.listenForEvents(baseUrl, credentials, eventTypes, eventTimeout);
                        break;
                    case 'getPlayerInfo':
                        result = await this.executeMinescriptCommand(baseUrl, credentials, '\\eval "minescript.player()"');
                        break;
                    case 'getWorldInfo':
                        result = await this.executeMinescriptCommand(baseUrl, credentials, '\\eval "minescript.world_info()"');
                        break;
                    case 'getInventory':
                        result = await this.executeMinescriptCommand(baseUrl, credentials, '\\eval "minescript.player_inventory()"');
                        break;
                    case 'getEntities':
                        const entityOptions = this.getNodeParameter('entityOptions', i, {});
                        result = await this.getEntities(baseUrl, credentials, entityOptions);
                        break;
                    case 'getBlockInfo':
                        const blockX = this.getNodeParameter('blockX', i);
                        const blockY = this.getNodeParameter('blockY', i);
                        const blockZ = this.getNodeParameter('blockZ', i);
                        result = await this.executeMinescriptCommand(baseUrl, credentials, `\\eval "minescript.getblock(${blockX}, ${blockY}, ${blockZ})"`);
                        break;
                    default:
    ...
    async startSession(baseUrl, credentials, sessionName) {
        const sessionId = this.generateUUID();
        // Send session start event
        const response = await axios_1.default.post(`${baseUrl}/events`, {
    ...
    async endSession(baseUrl, credentials) {
    ...
    async getSessionStatus(baseUrl, credentials) {
    ...
    async listenForEvents(baseUrl, credentials, eventTypes, timeout) {
        const sessionId = credentials.sessionId || this.generateUUID();
        return new Promise((resolve, reject) => {
            const events = [];
            const eventSource = new eventsource_1.EventSource(`${baseUrl}/events/stream?session_id=${sessionId}&types=${eventTypes.join(',')}`, {
                headers: {
                    'X-Client-ID': credentials.clientId,
                    'X-Auth-Key': credentials.authKey,
                },
    ...
    async getEntities(baseUrl, credentials, options) {
        let command = '\\eval "';
        ...
        if (options.range || options.limit || options.sortBy) {
            const filters = [];
            if (options.range)
                filters.push(`range=${options.range}`);
            if (options.limit)
                filters.push(`limit=${options.limit}`);
            if (options.sortBy)
                filters.push(`sort='${options.sortBy}'`);
            if (filters.length > 0) {
    ...
    async executeMinescriptCommand(baseUrl, credentials, command) {
        const sessionId = credentials.sessionId || this.generateUUID();
...
exports.MinecraftSession = MinecraftSession;
