...
export class MinescriptCommand implements INodeType {
			...
			try {
				...
				switch (operation) {
					case 'executeScript':
						...
						const scriptArgs = this.getNodeParameter('scriptArgs', i, '') as string;
					...
					case 'eval':
						...
						const additionalLines = this.getNodeParameter('additionalLines', i, {}) as any;
					...
					case 'chat':
						const chatMessage = this.getNodeParameter('message', i) as string;
						command = `\\eval "minescript.chat('${chatMessage.replace(/'/g, "\\'")}')"`; 
						break;

					case 'echo':
						const echoMessage = this.getNodeParameter('message', i) as string;
						command = `\\eval "minescript.echo('${echoMessage.replace(/'/g, "\\'")}')"`; 
						break;

					case 'execute':
						const minecraftCommand = this.getNodeParameter('minecraftCommand', i) as string;
						command = `\\eval "minescript.execute('${minecraftCommand.replace(/'/g, "\\'")}')"`;
						break;

					case 'copy':
						const x1 = this.getNodeParameter('copyX1', i) as number;
						const y1 = this.getNodeParameter('copyY1', i) as number;
						const z1 = this.getNodeParameter('copyZ1', i) as number;
						const x2 = this.getNodeParameter('copyX2', i) as number;
						const y2 = this.getNodeParameter('copyY2', i) as number;
						const z2 = this.getNodeParameter('copyZ2', i) as number;
						const copyLabel = this.getNodeParameter('copyLabel', i, '') as string;
						const noLimit = this.getNodeParameter('noLimit', i) as boolean;
					...
					case 'paste':
						const pasteX = this.getNodeParameter('pasteX', i) as number;
						const pasteY = this.getNodeParameter('pasteY', i) as number;
						const pasteZ = this.getNodeParameter('pasteZ', i) as number;
						const pasteLabel = this.getNodeParameter('pasteLabel', i, '') as string;
					...
					case 'screenshot':
						const screenshotName = this.getNodeParameter('screenshotName', i, '') as string;
						if (screenshotName) {
					...
					case 'jobs':
						const showAllJobs = this.getNodeParameter('showAllJobs', i) as boolean;
						if (showAllJobs) {
							command = '\\jobs all';
						} else {
							command = '\\jobs';
					...
					case 'killjob':
						const killJobId = this.getNodeParameter('jobId', i) as string;
						command = `\\killjob ${killJobId}`;
						break;

					case 'suspend':
						const suspendJobId = this.getNodeParameter('jobId', i, '') as string;
						if (suspendJobId) {
					...
					case 'resume':
						const resumeJobId = this.getNodeParameter('jobId', i, '') as string;
						if (resumeJobId) {
