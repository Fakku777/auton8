...
import com.april777.auton8.bridges.*;
import com.april777.auton8.minescript.MinescriptClient;
import com.april777.auton8.docker.DockerManager;
...
public final class Auton8Core {
    ...
    public Auton8Core(Config cfg) {
        ...
        this.cfg = cfg.copy();
        this.client = new MinescriptClient(this.cfg);
        this.dockerManager = new DockerManager(this.cfg.dockerProjectDir);

        // Create Baritone first so ChatBridge can forward local "#..." lines to it
        this.baritoneBridge   = new BaritoneBridge(this.cfg, client);

        // ChatBridge now receives the baritone reference (preserving existing functionality)
        this.chatBridge       = new ChatBridge(this.cfg, client, baritoneBridge);

        this.playerBridge     = new PlayerBridge(this.cfg, client);
        this.telemetryBridge  = new TelemetryBridge(this.cfg, client);
        this.connectionBridge = new ConnectionBridge(this.cfg, client);
        this.serverChatBridge = new ServerChatBridge(this.cfg, client);
        this.hudBridge        = new HudBridge(client, this.cfg);
        this.lifeBridge       = new LifeBridge(this.cfg, client);

        // Fan out commands arriving on cmd endpoint to relevant bridges.
        client.onMessage(this.cfg.cmdEndpoint, (endpoint, json) -> {
        ...
        client.onMessage(this.cfg.evtEndpoint, (endpoint, json) -> {
            try {
    ...
    public void enable() {
        ...
        if (cfg.autoStartDocker) {
            dockerManager.startN8nContainer();
        }
        ...
        // --- Hard stop any stale Baritone path (route via cmd endpoint) ---
        String cancel = "{\"type\":\"baritone_cmd\",\"cmd\":\"#cancel\",\"session_id\":\""
            + (cfg.sessionId == null ? "" : cfg.sessionId)
            + "\",\"ts\":" + System.currentTimeMillis() + "}";
        client.publish(cfg.cmdEndpoint, cancel);
    ...
    public void emitSessionStart() {
        ...
        client.publish(cfg.evtEndpoint, o.toString());
    }
    ...
    public void emitSessionEnd() {
        ...
        if (cfg.sessionId != null) o.addProperty("session_id", cfg.sessionId);
        o.addProperty("ts", System.currentTimeMillis());
        client.publishSync(cfg.evtEndpoint, o.toString(), 750);
    }
    ...
    public void requestOneShotTelemetry() {
        ...
        out.add("obs", obs);
        client.publish(cfg.evtEndpoint, out.toString());
    }
