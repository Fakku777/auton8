...
public class TimelapseModule extends Module {
    ...
    public TimelapseModule(Category category) {
    ...
    private void onTick(TickEvent.Post e) {
        ...
        long intervalMs = (long) Math.round(everySeconds.get() * 1000.0);
    ...
    private void takeScreenshot(MinecraftClient mc) {
        ...
        String fileName = filenamePrefix.get() + "_" + ts + ".png";

        Framebuffer fb = mc.getFramebuffer();

        Consumer<Text> messageSink = toastOnSave.get()
            ? (text) -> mc.inGameHud.getChatHud().addMessage(text)
            : (text) -> {};
        ...
        mc.execute(() -> {
            try {
                ...
                if (toastOnSave.get()) {
                    mc.inGameHud.getChatHud().addMessage(Text.literal("[Auton8] Timelapse: failed to save screenshot."));
                }
            }
        });
    }
