...
public final class ServerChatBridge implements Bridge {
    ...
    public ServerChatBridge(Config cfg, MinescriptClient client) {
        this.cfg = cfg;
        this.client = client;
    }
    ...
    /**
     * Accepted payloads on cfg.cmdEndpoint:
    ...
    public void onCommand(String json) {
        try {
            ...
            if (text == null) {
                client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("reject", "no_text"));
                return;
            }
            ...
            if (text.isEmpty() || text.length() > 256) {
                client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("reject", "bad_text"));
                return;
            }
            ...
            if (now - lastSendMs < MIN_INTERVAL_MS) {
                client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("reject", "rate_limited"));
                return;
            }
            lastSendMs = now;
            ...
            mc.execute(() -> {
                if (mc.player == null || mc.player.networkHandler == null) {
                    client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("reject", "no_player"));
                    return;
                }
                ...
                client.publish(cfg.evtEndpoint, JsonUtils.wrap("accepted", ack));
            });

        } catch (Exception e) {
            client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("error", e.getClass().getSimpleName()));
        }
    }
}
