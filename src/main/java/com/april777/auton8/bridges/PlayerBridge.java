...
public final class PlayerBridge implements Bridge {
    ...
    public PlayerBridge(Config cfg, MinescriptClient client) {
        this.cfg = cfg;
        this.client = client;
    }
        ...
        ClientTickEvents.END_CLIENT_TICK.register(mcClient -> {
            ...
            for (UUID id : diff(nextInside, inside)) {
                Snap s = last.get(id); if (s != null) {
                    this.client.publish(cfg.evtEndpoint, JsonUtils.playerSpotted(s.name, id.toString(), s.x, s.y, s.z, s.dist));
                }
            }
            // LEAVE outer (includes vanish/unload)
            for (UUID id : diff(inside, nextInside)) {
                Snap s = last.get(id); if (s != null) {
                    this.client.publish(cfg.evtEndpoint, JsonUtils.playerLeftRadius(s.name, id.toString(), s.x, s.y, s.z, s.dist));
                }
            }
            ...
            for (UUID id : diff(nextDanger, dangerInside)) {
                Snap s = last.get(id); if (s != null) {
                    this.client.publish(cfg.evtEndpoint, JsonUtils.playerDangerEnter(s.name, id.toString(), s.x, s.y, s.z, s.dist));
                }
            }
            // LEAVE danger (includes vanish/unload)
            for (UUID id : diff(dangerInside, nextDanger)) {
                Snap s = last.get(id); if (s != null) {
                    this.client.publish(cfg.evtEndpoint, JsonUtils.playerDangerLeft(s.name, id.toString(), s.x, s.y, s.z, s.dist));
                }
            }
