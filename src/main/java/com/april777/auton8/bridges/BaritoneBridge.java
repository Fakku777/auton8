...
public final class BaritoneBridge implements Bridge {
    ...
    public BaritoneBridge(Config cfg, MinescriptClient client) {
        this.cfg = cfg;
        this.client = client;
    ...
    public void enable() {
        ClientTickEvents.END_CLIENT_TICK.register(mc -> {
            try {
                ...
                samplePos(mc);
                baritone.tick();
                updateHeuristicState(mc);
                maybeDetectGoalOrStuck();
                planTick();
                maybePublishSnapshot();
            } catch (Exception e) {
                this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("error", e.getClass().getSimpleName()));
            }
        });
    }
    ...
    public void onCommand(String json) {
        ...
        try {
            ...
            this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("error", "plan_parse_" + e.getClass().getSimpleName()));
        }
    }
    ...
    private void handleSingleCmd(JsonObject j) {
        ...
        if (cmd == null || !cmd.startsWith("#") || cmd.length() > 120) {
            this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("cmd_reject","bad_cmd"));
            return;
        }
        ...
        mc.execute(() -> {
            if (mc.player == null || mc.player.networkHandler == null) {
                BaritoneBridge.this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("cmd_reject","no_player"));
                return;
            }
            ...
            BaritoneBridge.this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("accepted", send));
        });
    }
    ...
    private void updateHeuristicState(MinecraftClient mc) {
        ...
        if (awaitingAccept && movingFastEnough) {
            ...
            if (!acceptedEmitted && (baritonePathing || now - movingSinceMs >= ACCEPT_SUSTAIN_MS)) {
                this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("cmd_accepted", lastCmd == null ? "" : lastCmd));
        ...
        if (awaitingAccept && now > acceptDeadlineMs && !acceptedEmitted) {
            this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("cmd_reject", "timeout_no_pathing"));
            awaitingAccept = false;
        }
    ...
    private void maybeDetectGoalOrStuck() {
        ...
        if (target != null && distanceRemaining >= 0) {
            ...
            if (withinGoalNow && !goalEmittedForThisTarget && (now - withinGoalSinceMs) >= GOAL_STAY_MS && (now - lastGoalEmitMs) >= GOAL_REEMIT_DEBOUNCE) {
                ...
                this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("goal_reached", target.toShortString()));
        ...
        if (sinceHorizMove >= STUCK_IDLE_MS) {
            state = "STUCK";
            lastReason = "stuck";
            this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("stuck_detected", String.valueOf((int)Math.round(distanceRemaining))));
    ...
    private void emitPlanEvent(String name, JsonObject payload) {
        this.client.publish(cfg.evtEndpoint, JsonUtils.wrap(name, payload));
    }
    ...
    private void maybePublishSnapshot() {
        ...
        String topic = cfg.baritoneStateEndpointOrDefault();
        this.client.publish(topic, JsonUtils.wrap("baritone_state", snap));
    }
    ...
    private void sendClientChat(String msg, BlockPos newTarget, boolean expectMovement) {
        ...
        mc.execute(() -> {
            if (mc.player != null && mc.player.networkHandler != null) {
                ...
                BaritoneBridge.this.client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("accepted", msg));
