...
public final class TelemetryBridge implements Bridge {
    ...
    public TelemetryBridge(Config cfg, MinescriptClient client) {
        this.cfg = cfg;
        this.client = client;
    }
    ...
    private void publishTelemetryIfAvailable() {
        ...
        if (lastDimension == null || !lastDimension.equals(dimension)) {
            ...
            client.publish(cfg.evtEndpoint, JsonUtils.baseEvent("dimension_changed", dimension));
        }

        // telemetry snapshot
        client.publish(cfg.evtEndpoint,
